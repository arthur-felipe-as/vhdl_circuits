...
signal enable_display: std_logic;	
	...
	mostrar_entrada : display7_num port map (botoes_bin, saida_input);
	mostrar_saida : display7_char port map (botoes_bin, saida_output);
	
	
	saida_input  <= "1111110" when enable_display = '0' else saida_input;
   saida_output <= "1111110" when enable_display = '0' else saida_output;
 end behavior;